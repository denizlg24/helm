  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        ...
        if (multiSelections.length >= 2) {
          if (
            ...
            if (deduped.length < 2) {
              ...
              if (deduped.length === 1 && only) {
                requestAnimationFrame(() =>
                  textarea.setSelectionRange(only.start, only.end)
                )
              }
            } else {
      ...
      if (e.key === "Enter" && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
        ...
        if (unorderedMatch) {
          const fullMatch = unorderedMatch[0]
        ...
        if (orderedMatch) {
          const fullMatch = orderedMatch[0]
          const numStr = orderedMatch[2] ?? "0"
          const afterMarker = lineText.slice(fullMatch.length)
