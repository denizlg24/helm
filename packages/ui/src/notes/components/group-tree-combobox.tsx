...
export function GroupTreeCombobox({
  ...
  const renderNode = (group: GroupLike, depth: number): React.ReactNode => {
    if (!visibility.get(group.id)) return null

    const children = (childrenByParent.get(group.id) ?? []).filter((child) =>
      visibility.get(child.id)
    )
    const hasChildren = children.length > 0
    const expanded =
      normalizedQuery.length > 0 ? true : expandedIds.has(group.id)
    const selected = value.includes(group.id)

    return (
      <div key={group.id} className="min-w-0 overflow-hidden">
        <div
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-[min(22rem,calc(100vw-2rem))] max-w-[calc(100vw-2rem)] overflow-hidden p-0 pb-2"
        align="start"
        sideOffset={6}
      >
